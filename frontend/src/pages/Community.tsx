...
export default function Community() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-800 dark:text-slate-200 pb-20 relative">
      ...
      {confirmModal.isOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in px-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 max-w-sm w-full shadow-2xl scale-100 animate-in zoom-in-95">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-center mb-8 font-medium">Expulsarás a <span className="text-yellow-500 font-bold uppercase">{confirmModal.alias}</span>. Esta acción no se deshace.</p>
      ...
      <div className="max-w-350 mx-auto px-4 sm:px-6 lg:px-8 pt-10 md:pt-16">
        ...
        <header className="mb-10 md:mb-16">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white tracking-tighter mb-4 italic uppercase">
        ...
        <Tabs defaultValue="search" className="flex flex-col lg:flex-row gap-8 items-start w-full">
          ...
          <div className="w-full lg:w-72 shrink-0">
            ...
            <TabsList className="bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 p-2 rounded-2xl w-full flex flex-row lg:flex-col h-auto overflow-x-auto lg:overflow-visible gap-2 shadow-xl custom-scrollbar justify-start">

              <TabsTrigger value="search" className="flex-1 lg:w-full justify-center lg:justify-start py-4 px-4 rounded-xl font-black text-xs md:text-sm uppercase tracking-wider data-[state=active]:bg-yellow-500 data-[state=active]:text-black text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-all">
            ...
            <div className="hidden lg:block mt-6 p-6 rounded-2xl bg-slate-100 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50">
              <ShieldCheck className="w-8 h-8 text-yellow-500/50 mb-3" />
              <h4 className="text-slate-900 dark:text-white font-bold mb-1">Zona Segura</h4>
          ...
          <div className="flex-1 w-full min-w-0">
            ...
            <TabsContent value="search" className="m-0 space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1 group">
                  ...
                  <Input 
                    ...
                    className="h-16 bg-white dark:bg-slate-800/90 border-slate-200 dark:border-slate-600 pl-14 text-lg font-bold rounded-2xl focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 text-slate-900 dark:text-white shadow-md"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {searchResults.map((user: any) => (
                  <Card key={user.id} className="bg-white dark:bg-slate-800/80 border-slate-200 dark:border-slate-600 hover:border-yellow-500 shadow-xl transition-all rounded-3xl overflow-hidden group">
                    <CardContent className="p-6 flex flex-col items-center text-center">
                      <div className="w-24 h-24 rounded-2xl border-2 border-slate-200 dark:border-slate-700 group-hover:border-yellow-500/50 transition-colors overflow-hidden mb-5 bg-slate-100 dark:bg-slate-900">
                        <img src={getImageUrl(user.picture) || ''} className="w-full h-full object-cover" alt={user.alias} />
                      </div>
                      <p className="font-black text-xl text-slate-900 dark:text-white uppercase tracking-tight mb-5 truncate w-full">{user.alias}</p>
                      <Button onClick={() => sendRequest(user.id)} variant="outline" className="w-full h-12 border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-yellow-500 hover:text-black hover:border-yellow-500 font-bold rounded-xl gap-2 transition-all">
            ...
            <TabsContent value="requests" className="m-0 space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
               {requests.length === 0 ? (
                 <div className="text-center py-20 bg-slate-100 dark:bg-slate-800/30 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700">
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                   {requests.map((user: any) => (
                     <Card key={user.id} className="bg-white dark:bg-slate-800/90 border-slate-200 dark:border-slate-600 hover:border-yellow-500/50 rounded-2xl shadow-lg transition-all">
                       <CardContent className="p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
                         <div className="flex items-center gap-4 w-full sm:w-auto">
                           <img src={getImageUrl(user.picture) || ''} className="w-14 h-14 rounded-full border-2 border-yellow-500/30 bg-slate-100 dark:bg-slate-900 object-cover shrink-0" alt={user.alias} />
                           <div className="text-left">
                             <p className="font-black text-slate-900 dark:text-white text-lg leading-tight truncate">{user.alias}</p>
            ...
            <TabsContent value="friends" className="m-0 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {friends.length === 0 ? (
                  <div className="col-span-full text-center py-20 bg-slate-100 dark:bg-slate-800/30 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700">
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-sm">Tu equipo está vacío</p>
                  </div>
                ) : (
                  friends.map((user: any) => (
                    <Card key={user.id} className="bg-white dark:bg-slate-800/90 border-slate-200 dark:border-slate-600 hover:border-yellow-500/50 shadow-xl rounded-3xl relative group overflow-hidden">
                      <CardContent className="p-8 flex flex-col items-center">
                        <Link to={`/friend/${user.id}`} className="flex flex-col items-center w-full">
                          <div className="relative mb-5">
                            <div className="w-24 h-24 rounded-full border-4 border-yellow-500 overflow-hidden shadow-lg bg-slate-100 dark:bg-slate-900 group-hover:scale-105 transition-transform">
                              <img src={getImageUrl(user.picture) || ''} className="w-full h-full object-cover" alt={user.alias} />
                            </div>
                            <ShieldCheck className="absolute bottom-0 right-0 w-8 h-8 text-yellow-500 fill-slate-900" />
                          </div>
                          <p className="font-black text-xl text-slate-900 dark:text-white uppercase tracking-tight mb-2 truncate w-full text-center group-hover:text-yellow-500 dark:group-hover:text-yellow-400 transition-colors">{user.alias}</p>
                        </Link>
